...
(function (EPKLib) {
  ...
  // --- RECONSTRUCTED & DYNAMIC CDN COMPATIBILITY FUNCTIONS ---
  ...
  function getParentUrl(path) {
    const baseHref = "https://cdn.jsdelivr.net/gh/hrn-client/1.8.8/";
    
    let url;
    if (path.includes("cdn.jsdelivr.net") || path.startsWith("./") || !path.includes("://")) {
      url = new URL(path, baseHref);
    } else {
  ...
  function joinUrls(baseUrl, pathSegment) {
    ...
    if (base.startsWith(window.location.origin)) {
      const path = base.substring(window.location.origin.length);
      const cdnBase = "https://cdn.jsdelivr.net/gh/hrn-client/1.8.8";
      base = cdnBase + (path.startsWith('/') ? path : '/' + path);
    }
